...
>
> 🪶 **极致轻量。** ARIS 就是一堆 Markdown 文件——没有框架、没有数据库、没有 Docker、没有守护进程。每个 skill 就是一个 `SKILL.md`，任何 LLM 系统都能读、能改、能跑。今天用 Claude Code，明天换 [OpenClaw](docs/OPENCLAW_ADAPTATION.md)，后天接你自己的 agent——思想可迁移，不绑定任何平台。
