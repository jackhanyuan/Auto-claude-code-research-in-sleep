...
- **2026-04-30** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 📝 **`/rebuttal` per-reviewer thread mode + transferable patterns from real rebuttal experience** ([SKILL.md](skills/rebuttal/SKILL.md)) — adds `VENUE_MODE` (`single_document` | `per_reviewer_thread`) for venues where each reviewer thread renders independently (OpenReview-style), `reviewer_priority: pivotal` routing, `structural_distinction` response mode (against "your method reduces to X" attacks, with explicit anti-rhetoric guard), reusable `SETUP_METRICS_BLOCK.md` to prevent prose drift across reviewer threads, **5 new reviewer-defensive heuristics** (minimum sufficient evidence, pre-registered calibration phrasing, surface non-obvious design choices upfront, structural distinction over denial, concede without surrendering), 2 new Phase 5 lints (thread-local context + adversarial design-choice scan), severity-scaled stress rounds (focused follow-ups on `pivotal` reviewers, hard-cap at 5), and Phase 7 output branching. Each edit survived **two rounds of Codex MCP cross-model review** (`gpt-5.5` xhigh, fresh thread for round 1) before merge, plus a third post-merge audit pass that surfaced and fixed 3 single-document assumptions that had slipped through the per-reviewer-thread mode-gating. Default `VENUE_MODE = single_document` keeps ICML-style behavior — **zero change for existing users**. Highest-priority follow-up tracked separately: maintainer to verify no downstream skill parses `ISSUE_BOARD.md` in a way that breaks on the new `reviewer_priority` field
- **2026-04-30** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🪞 **Codex skill mirror rebuilt + dedicated install/update chain** ([#179](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/179), community contribution by [@No-518](https://github.com/No-518)). Treats `skills/skills-codex/` as a Claude-mainline mirror/adaptation layer rather than a separate product line: completes coverage to **all 67 mainline skills**, replaces the legacy `mcp__codex__codex` reviewer path with **Codex-native `spawn_agent` + `send_input`**, and reworks `training-check` as interactive current-session monitoring. New [`tools/install_aris_codex.sh`](tools/install_aris_codex.sh) (project-local symlink installs under `.agents/skills/<name>`, manifest at `.aris/installed-skills-codex.txt`, dry-run / reconcile / uninstall, optional Claude/Gemini reviewer overlays) and [`tools/smart_update_codex.sh`](tools/smart_update_codex.sh) (copied-install updates with `Needs merge` detection for local customizations). Anti-drift: `tests/test_codex_skill_mirror.py` enforces exact mirror coverage; `tests/test_codex_install_update.py` covers 26 install/update failure paths (dry-run, conflict, reconcile, stale-link, symlink-refusal, overlay). Reviewed via the ARIS cross-model protocol (GPT-5.5 xhigh via Codex MCP). One open discussion item tracked in [#184](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/184): two reviewer-heavy skills declare `REVIEWER_MODEL = gpt-5.4` at the top but spawn the reviewer without an explicit `model:` field — whether `spawn_agent` fresh-context delegation is sufficient cross-model independence (vs. requiring explicit `model:` to prevent silent same-model self-review drift) is intentionally left for community/maintainer discussion before deciding to fix
